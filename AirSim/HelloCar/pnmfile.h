...
static image<rgb> *charPtrToImage(const char *img, int width, int height) {
	image<rgb> * im = new image<rgb>(width, height);
	memcpy(im->data, img, width * height * sizeof(rgb));

	return im;
}

static char *imageToCharPtr(image<rgb> *im, int width, int height) {
	char * img = new char[width * height * 3];
	printf("rgb = %d\n", width * height * sizeof(rgb));
	printf("3 = %d\n", width * height * 3);
	memcpy(img, im->data, width * height * sizeof(rgb));
